...
export class TemplateQuery extends CrudService<
  Template,
  Template | CreateMachineTemplateWithAttributesDto
> {
  ...
  useCreateTemplateWithAttributes() {
    ...
    >({
      mutationFn: (data) =>
        axios.post(`${this.baseUrl}/with-attributes`, data).then((r) => r.data),
      onSuccess: () =>
  ...
  useDuplicate() {
    return useMutation({
      mutationFn: (id: number) =>
        axios.post(`/api/machine-templates/${id}/duplicate`),
      onSuccess: () =>
        this.queryClient.invalidateQueries({ queryKey: [this.baseUrl] }),
    });
  }
